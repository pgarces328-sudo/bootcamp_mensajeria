...
export default function App(): React.JSX.Element {
  ...
  // PASO 1: operaciones básicas MMKV, sincrónicas sin await
  useEffect(() => {
  ...
  // PASO 2: hooks reactivos, persisten sin await
  const [darkModeRaw, setDarkMode] = useMMKVBoolean(KEYS.DARK_MODE, storage);
  const [themeRaw] = useMMKVString(KEYS.THEME, storage);

  const darkMode: boolean = darkModeRaw ?? false;
  const theme: string = themeRaw ?? 'dark';

  // PASO 3: SecureStore token cifrado
  const [tokenStatus, setTokenStatus] = useState<string>('Sin token guardado');
  ...
  const [isBenchRunning, setIsBenchRunning] = useState<boolean>(false);
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: bg }]}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={[styles.card, { backgroundColor: card }]}>
          ...
          {benchResult && <Text style={[styles.bench, { color: fg }]}>{benchResult}</Text>}
          <Pressable style={styles.btn} onPress={runBenchmark} disabled={isBenchRunning}>
            {isBenchRunning ? (
