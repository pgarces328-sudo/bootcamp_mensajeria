...
import {
  ShipmentsStackNavigator,
} from './ShipmentsStackNavigator';
import type {
  RootTabParamList,
} from './types';
...
export function AppNavigatorTabs(): React.JSX.Element {
  const trackedCount = usePackageStore(
    (s) => s.trackedIds.length
  );
    ...
    <Tab.Navigator
      screenOptions={({
        ...
        tabBarIcon: ({
          ...
          if (
            route.name === 'ShipmentsTab'
          ) {
            icon = focused
              ? 'cube'
              : 'cube-outline';
          } else if (
            route.name === 'TrackedTab'
          ) {
            icon = focused
              ? 'star'
              : 'star-outline';
          } else if (
            route.name === 'DriversTab'
          ) {
            icon = focused
              ? 'people'
              : 'people-outline';
