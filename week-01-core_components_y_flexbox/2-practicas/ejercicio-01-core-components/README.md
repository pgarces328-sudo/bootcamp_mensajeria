...
Construirás una tarjeta de perfil de usuario paso a paso, descomentando código y
verificando el resultado en Expo Go después de cada paso.
...
- Usar `Pressable` para crear una acción con feedback visual.

## 📋 Requisitos previos

- Haber leído `02-core-components.md`.
...
**Verifica:** el avatar circular debe aparecer sobre el nombre.

> 💡 `borderRadius: 60` en un cuadrado de 120×120 lo convierte en círculo perfecto
...
**Verifica:** tres columnas distribuidas con `justifyContent: 'space-around'`.
...
**Verifica:** el botón debe cambiar de opacidad al presionarlo. Revisa la consola
de Metro para ver el mensaje.
...
1. Cambia el `resizeMode` del avatar de `'cover'` a `'contain'` — ¿qué cambia?
2. Modifica `numberOfLines={3}` de la biografía a `numberOfLines={1}` — ¿qué pasa?
3. Reemplaza `Pressable` con `TouchableOpacity` — ¿el comportamiento es idéntico?